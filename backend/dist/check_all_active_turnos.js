...
async function checkAllActive() {
    const activeShifts = await db_1.default.turno.findMany({
        where: { activo: true },
        include: {
            cuentas: {
                where: { estado: 'PAGADA' },
                ...
                c.divisionesCuentas.forEach(d => {
                    if (!d.turno_pago_id) {
                        if (d.metodo_pago === 'EFECTIVO')
                            ef += Number(d.monto_proporcional);
                        else if (d.metodo_pago === 'MIXTO')
                            ef += Number(d.monto_efectivo);
                    }
                });
            }
            else {
