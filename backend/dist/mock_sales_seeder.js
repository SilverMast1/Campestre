...
async function main() {
    ...
    await db_1.default.divisionCuenta.deleteMany({});
    await db_1.default.detalleCuenta.deleteMany({});
    await db_1.default.cuenta.deleteMany({});
    await db_1.default.movimientoInventario.deleteMany({});
    await db_1.default.retiroCaja.deleteMany({});
    await db_1.default.turno.deleteMany({});
    await db_1.default.asignacionCadiCliente.deleteMany({});
    await db_1.default.cadi.deleteMany({});
    await db_1.default.cliente.deleteMany({});
    console.log('Tablas limpiadas.');
    // 2. Crear socios de prueba
    console.log('Creando socios...');
    const socio1 = await db_1.default.cliente.create({
        data: {
    ...
    const socio2 = await db_1.default.cliente.create({
        data: {
    ...
    const socio3 = await db_1.default.cliente.create({
        data: {
    ...
    const cadi1 = await db_1.default.cadi.create({
        data: {
    ...
    const cadi2 = await db_1.default.cadi.create({
        data: {
    ...
    const crearCuentaSimulada = async (params) => {
        ...
        for (const item of params.items) {
            ...
            const precio = new decimal_js_1.Decimal(prod.precio_venta);
            const itemSubtotal = precio.mul(item.cantidad);
            subtotal = subtotal.plus(itemSubtotal);
            lineas.push({
                producto_id: prod.id,
                cantidad: new decimal_js_1.Decimal(item.cantidad),
        ...
        // Crear cuenta madre
        const cuenta = await db_1.default.cuenta.create({
            data: {
        ...
        for (const lin of lineas) {
            await db_1.default.detalleCuenta.create({
                data: {
                    cuenta_id: cuenta.id,
        ...
        for (const pago of params.pagos) {
            if (pago.cliente_id) {
                await db_1.default.divisionCuenta.create({
                    data: {
                        cuenta_id: cuenta.id,
                        cliente_id: pago.cliente_id,
                        porcentaje_participacion: new decimal_js_1.Decimal(pago.monto).div(total).mul(100),
                        monto_proporcional: new decimal_js_1.Decimal(pago.monto),
    ...
    const crearTurnoSimulado = async (params) => {
        return await db_1.default.turno.create({
            data: {
                usuario_id: usuario.id,
                fondo_inicial: new decimal_js_1.Decimal(params.fondo_inicial),
                caja_efectivo: new decimal_js_1.Decimal(0),
                caja_tarjeta: new decimal_js_1.Decimal(0),
                caja_cargos: new decimal_js_1.Decimal(0),
    ...
    const actualizarTurnoTotales = async (turnoId, fondo, cuentasList) => {
        ...
        for (const cta of cuentasList) {
            for (const pago of cta.pagos) {
                const monto = new decimal_js_1.Decimal(pago.monto);
                if (pago.metodo === 'EFECTIVO')
                    efectivo = efectivo.plus(monto);
                else if (pago.metodo === 'TARJETA')
                    tarjeta = tarjeta.plus(monto);
                else if (pago.metodo === 'CARGO_SOCIO')
                    cargos = cargos.plus(monto);
            }
        }
        await db_1.default.turno.update({
            where: { id: turnoId },
            data: {
