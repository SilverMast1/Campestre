...
exports.listarProductosPorArea = listarProductosPorArea;
exports.abrirCuenta = abrirCuenta;
exports.guardarConsumos = guardarConsumos;
exports.previsualizarSplit = previsualizarSplit;
exports.pagarYCerrarCuenta = pagarYCerrarCuenta;
exports.ajustarStockArea = ajustarStockArea;
exports.listarTodasLasCuentas = listarTodasLasCuentas;
exports.eliminarCuenta = eliminarCuenta;
exports.resetearDatos = resetearDatos;
exports.crearProducto = crearProducto;
exports.obtenerBalanceCaja = obtenerBalanceCaja;
exports.actualizarMetodoPagoCuenta = actualizarMetodoPagoCuenta;
exports.transferirStock = transferirStock;
exports.listarTodosLosProductos = listarTodosLosProductos;
exports.registrarMermaStock = registrarMermaStock;
exports.listarMermas = listarMermas;
exports.eliminarProducto = eliminarProducto;
exports.fusionarCuentas = fusionarCuentas;
exports.cambiarAreaCuenta = cambiarAreaCuenta;
const decimal_js_1 = require("decimal.js");
const db_1 = __importDefault(require("../db"));
const cache_1 = require("../cache");
// 1. Obtener productos y stock de un área específica (Bar, Snack, Palapa)
async function gestionarStock(tx, areaId, productoId, cantidadDiff, usuarioId, cuentaId) {
    if (cantidadDiff.isZero())
        return;
    ...
    if (cantidadDiff.greaterThan(0)) {
        ...
        if (nombreProd === 'agua mineral prep') {
            ...
            if (!inv)
                throw new Error(`Producto ${prod.nombre} no registrado en el inventario de esta área`);
            const stockActual = inv.stock;
            if (stockActual.lessThan(cantidadDiff))
                throw new Error(`Stock insuficiente para ${prod.nombre}. Disp: ${stockActual}, Req: ${cantidadDiff}`);
...
async function listarProductosPorArea(req, res) {
    ...
    try {
        const productosConStock = await db_1.default.inventarioArea.findMany({
        ...
        (0, cache_1.setCache)(cacheKey, productos, 90);
...
async function abrirCuenta(req, res) {
    ...
    try {
        ...
        const nuevaCuenta = await db_1.default.cuenta.create({
            data: {
                ...
                impuestos: new decimal_js_1.Decimal(0),
                descuento: new decimal_js_1.Decimal(0),
                total: new decimal_js_1.Decimal(0),
            },
            include: {
                cadi: true,
                cliente: true,
...
async function guardarConsumos(req, res) {
    ...
    try {
        const cuenta = await db_1.default.cuenta.findUnique({ where: { id: cuentaId } });
        ...
        const cuentaActualizada = await db_1.default.$transaction(async (tx) => {
            ...
            for (const dp of detallesPrevios) {
                if (!prevQtys[dp.producto_id])
                    prevQtys[dp.producto_id] = new decimal_js_1.Decimal(0);
                prevQtys[dp.producto_id] = prevQtys[dp.producto_id].plus(new decimal_js_1.Decimal(dp.cantidad));
            }
            if (otraCuenta) {
            ...
            for (const p of productosFinales) {
                if (!newQtys[p.producto_id])
                    newQtys[p.producto_id] = new decimal_js_1.Decimal(0);
                newQtys[p.producto_id] = newQtys[p.producto_id].plus(new decimal_js_1.Decimal(p.cantidad));
            }
            const allProdIds = new Set([...Object.keys(prevQtys).map(Number), ...Object.keys(newQtys).map(Number)]);
            for (const prodId of allProdIds) {
                const prev = prevQtys[prodId] || new decimal_js_1.Decimal(0);
                const cur = newQtys[prodId] || new decimal_js_1.Decimal(0);
            ...
            let subtotalAcumulado = new decimal_js_1.Decimal(0);
            ...
            for (const p of productosFinales) {
                ...
                const cantidadDec = new decimal_js_1.Decimal(p.cantidad);
                const precioDec = p.precio_unitario !== undefined && p.precio_unitario !== null
                    ? new decimal_js_1.Decimal(p.precio_unitario)
                    : new decimal_js_1.Decimal(prod.precio_venta);
            ...
            const descuentoAcumulado = tieneDescuentoGlobal ? subtotalAcumulado.mul(0.30).toDP(2) : new decimal_js_1.Decimal(0);
            const totalDec = subtotalAcumulado.minus(descuentoAcumulado);
            const impuestos = new decimal_js_1.Decimal(0);
            ...
            if (cuenta.estado === 'PAGADA') {
                ...
                for (const div of divisiones) {
                    const porcentaje = new decimal_js_1.Decimal(div.porcentaje_participacion);
...
async function previsualizarSplit(req, res) {
    const cuentaId = parseInt(req.params.cuentaId);
    try {
        const cuenta = await db_1.default.cuenta.findUnique({
            where: { id: cuentaId },
            include: {
        ...
        const asignaciones = await db_1.default.asignacionCadiCliente.findMany({
...
async function pagarYCerrarCuenta(req, res) {
    const cuentaId = parseInt(req.params.cuentaId);
        ...
        await db_1.default.$transaction(async (tx) => {
            if (esPagoDirecto) {
                ...
                if (metodo_pago === 'MIXTO') {
                    ...
                    const totalCuenta = new decimal_js_1.Decimal(cuenta.total);
                    const efDec = new decimal_js_1.Decimal(monto_efectivo);
                    const tjDec = new decimal_js_1.Decimal(monto_tarjeta);
                ...
                const totalCuenta = new decimal_js_1.Decimal(cuenta.total);
                const abonoDec = new decimal_js_1.Decimal(abono);
                ...
                const totalCuenta = new decimal_js_1.Decimal(cuenta.total);
                let sumaPagos = new decimal_js_1.Decimal(0);
                for (const pago of pagos) {
                    const montoDec = new decimal_js_1.Decimal(pago.monto);
                    ...
                    let efDec = new decimal_js_1.Decimal(0);
                    let tjDec = new decimal_js_1.Decimal(0);
                    if (esMixto) {
                        ...
                        efDec = new decimal_js_1.Decimal(pago.monto_efectivo);
                        tjDec = new decimal_js_1.Decimal(pago.monto_tarjeta);
...
async function ajustarStockArea(req, res) {
    ...
    try {
        ...
        const nuevoStockDec = new decimal_js_1.Decimal(nuevo_stock);
        ...
        const resultado = await db_1.default.$transaction(async (tx) => {
            ...
            const stockAnterior = new decimal_js_1.Decimal(inv.stock);
            // Actualizar stock
        ...
        (0, cache_1.invalidateCache)('productos');
        return res.json({
...
async function listarTodasLasCuentas(req, res) {
    try {
...
async function crearProducto(req, res) {
    ...
    try {
        const { Decimal } = await Promise.resolve().then(() => __importStar(require('decimal.js')));
        const producto = await db_1.default.$transaction(async (tx) => {
            // Crear el producto
...
async function obtenerBalanceCaja(req, res) {
    try {
        ...
        const directasPorMetodo = await db_1.default.cuenta.groupBy({
            by: ['metodo_pago'],
            _sum: { total: true },
            where: {
                turno_id: turno.id,
        ...
        directasPorMetodo.forEach(group => {
            ...
            if (group.metodo_pago === 'EFECTIVO')
                efectivo += total;
            else if (group.metodo_pago === 'TARJETA')
                tarjeta += total;
            else if (group.metodo_pago === 'TRANSFERENCIA')
                transferencia += total;
            else if (group.metodo_pago === 'CARGO_SOCIO')
                cargo_socio += total;
        });
        // Cuentas mixtas directas
        const directasMixtas = await db_1.default.cuenta.aggregate({
        ...
        const splitsPorMetodo = await db_1.default.divisionCuenta.groupBy({
            by: ['metodo_pago'],
        ...
        const splitsMixtos = await db_1.default.divisionCuenta.aggregate({
            _sum: { monto_efectivo: true, monto_tarjeta: true },
            where: {
                cuenta: {
                    turno_id: turno.id,
        ...
        const divisionesPagadasDiferidas = await db_1.default.divisionCuenta.groupBy({
            by: ['metodo_pago'],
            _sum: { monto_proporcional: true },
        ...
        const retirosIngresos = await db_1.default.retiroCaja.groupBy({
            by: ['tipo'],
            _sum: { monto: true },
...
async function actualizarMetodoPagoCuenta(req, res) {
    const cuentaId = parseInt(req.params.cuentaId);
    ...
    try {
        ...
        await db_1.default.$transaction(async (tx) => {
            if (divisiones && Array.isArray(divisiones)) {
                ...
                let ef = new decimal_js_1.Decimal(0);
                let tj = new decimal_js_1.Decimal(0);
                if (metodo_pago === 'EFECTIVO') {
                    ef = new decimal_js_1.Decimal(cuenta.total);
                }
                else if (metodo_pago === 'TARJETA') {
                    tj = new decimal_js_1.Decimal(cuenta.total);
                }
                else if (metodo_pago === 'MIXTO') {
                    if (monto_efectivo !== undefined && monto_tarjeta !== undefined) {
                        ef = new decimal_js_1.Decimal(monto_efectivo);
                        tj = new decimal_js_1.Decimal(monto_tarjeta);
                    }
                    else {
...
async function transferirStock(req, res) {
    ...
    const cantidadDec = new decimal_js_1.Decimal(cantidad);
    ...
    try {
        const result = await db_1.default.$transaction(async (tx) => {
            ...
            const stockOrigen = new decimal_js_1.Decimal(invOrigen.stock);
            ...
            const stockDestino = new decimal_js_1.Decimal(invDestino.stock);
...
async function listarTodosLosProductos(req, res) {
    const cached = (0, cache_1.getCache)('productos_todos');
    if (cached)
        return res.json(cached);
    try {
        ...
        (0, cache_1.setCache)('productos_todos', resultado, 90);
        return res.json(resultado);
    }
    catch (error) {
...
async function registrarMermaStock(req, res) {
    ...
    try {
        ...
        const resultado = await db_1.default.$transaction(async (tx) => {
            ...
            const stockActual = new decimal_js_1.Decimal(inv.stock);
            const cantidadMermarDec = new decimal_js_1.Decimal(cantidadMermar);
...
async function listarMermas(req, res) {
    try {
        const mermas = await db_1.default.movimientoInventario.findMany({
...
async function fusionarCuentas(req, res) {
    ...
    try {
        ...
        const cuentaOrigen = await db_1.default.cuenta.findUnique({
            where: { id: parseInt(cuenta_origen_id) },
            include: { detalleCuentas: true }
        });
        const cuentaDestino = await db_1.default.cuenta.findUnique({
...
async function cambiarAreaCuenta(req, res) {
    const cuentaId = parseInt(req.params.cuentaId);
    ...
    try {
        ...
        const turnoDestino = await db_1.default.turno.findFirst({
