...
async function checkActive() {
                ...
                cuenta.divisionesCuentas.forEach(div => {
                    if (!div.turno_pago_id) {
                        if (div.metodo_pago === 'EFECTIVO')
                            efectivoVentas += Number(div.monto_proporcional);
                        else if (div.metodo_pago === 'MIXTO')
                            efectivoVentas += Number(div.monto_efectivo);
                    }
                });
            }
            else {
                if (cuenta.metodo_pago === 'EFECTIVO')
                    efectivoVentas += Number(cuenta.total);
                else if (cuenta.metodo_pago === 'MIXTO')
                    efectivoVentas += Number(cuenta.monto_efectivo);
            }
        }
    });
    ...
    activeShift.divisiones_pagadas.forEach(div => {
        if (div.metodo_pago === 'EFECTIVO')
            efectivoDivisionesPagadas += Number(div.monto_proporcional);
    });
