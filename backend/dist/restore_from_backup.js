...
async function main() {
    const backupPath = path_1.default.join(__dirname, '../../campestre_backup.sql');
    ...
    const content = fs_1.default.readFileSync(backupPath, 'utf16le');
            ...
            if (currentTable) {
                if (valuesBatch.length > 0) {
                    ...
                    for (let i = 0; i < valuesBatch.length; i += chunkSize) {
                        ...
                        await prisma.$transaction(insertQueries.map(q => prisma.$executeRawUnsafe(q)));
                    }
