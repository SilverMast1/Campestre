...
async function main() {
    ...
    const dbDirect = await db_1.prisma.cuenta.findMany({
        where: {
            estado: 'ABIERTA',
            area: { nombre: 'DEUDAS' },
    ...
    const dbClientBalances = new Map();
    ...
    await db_1.prisma.$transaction(async (tx) => {
        ...
        for (const [dbName, data] of dbClientBalances.entries()) {
            ...
            const currentDbBal = data.divs.reduce((sum, d) => sum.plus(new decimal_js_1.Decimal(d.monto_proporcional)), new decimal_js_1.Decimal(0))
                .plus(data.directs.reduce((sum, c) => sum.plus(new decimal_js_1.Decimal(c.total)), new decimal_js_1.Decimal(0)))
                .toNumber();
            ...
            if (excelSaldo === 0) {
                ...
                let remaining = new decimal_js_1.Decimal(excelSaldo);
                ...
                for (let i = 0; i < data.divs.length; i++) {
                    ...
                    if (i === 0) {
                        ...
                        remaining = new decimal_js_1.Decimal(0);
                    }
                    else {
