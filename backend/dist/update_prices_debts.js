...
async function main() {
    ...
    for (const item of PRECIOS) {
        ...
        const precioDec = new decimal_js_1.Decimal(item.precio);
        if (prods.length > 0) {
            ...
            for (const areaId of areaIds) {
                await prisma.inventarioArea.upsert({
                    ...
                    create: {
                        area_id: areaId,
                        producto_id: nuevo.id,
                        stock: new decimal_js_1.Decimal(20),
                        stock_minimo: new decimal_js_1.Decimal(5),
                        stock_maximo: new decimal_js_1.Decimal(50),
    ...
    for (const deuda of DEUDAS) {
        ...
        if (deuda.monto > 0) {
            ...
            const totalDec = new decimal_js_1.Decimal(deuda.monto);
